...
Workers reserve jobs using `reserveBlocking`, send heartbeats, and on success attempt atomic in-group chaining via `completeAndReserveNext`.
...
- Reserves jobs via `reserveBlocking` and emits events for lifecycle changes
